...
def test_for_best_classifier_train_data(all_table_keys, meta_values, meta_type):
    ...
    if not all_table_keys:
        train_df = train_df.drop(['w_human', 'w_landscape', 'w_building', 'w_edges', 'w_frequency', 'w_grayscale'],
                                 axis=1)
...
def create_lg_param_grid(params):
    ...
    for param_combination in param_grid:
        if param_combination['penalty'] == 'l2' and param_combination['solver'] in ['newton-cg', 'lbfgs', 'liblinear',
                                                                                    'saga']:
...
def create_file(filename):
    if not os.path.exists(filename):
        with open(filename, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["model", "all_table_keys", "meta_values", "tn", "fp", "fn", "tp", "accuracy", "precision", "recall",
                 "f1"])
...
def use_data_from_test_for_train_and_train_model(all_table_keys, meta_values, meta_type):
    ...
    if all_table_keys and meta_values:
        ...
        gbc_model = GradientBoostingClassifier(random_state=1, learning_rate=learning_rate, max_depth=max_depth,
        ...
        rfc_model = RandomForestClassifier(random_state=1, bootstrap=bootstrap, n_estimators=n_estimators,
        ...
        rfc_model = RandomForestClassifier(random_state=1, bootstrap=bootstrap, max_depth=max_depth,
...
def get_train_images_for_robustheit():
    ...
    for var in variants:
        ...
        for img_path in matched["img"]:
            ...
            relative = get_relative_subpath(normalize(img_path))
